...
public class DrippyOverlayScreen extends ModernScreen {
    ...
    private void drawCustomizationHint(GuiGraphics graphics) {
        ...
        graphics.pose().translate(arrowTailX + CUSTOMIZATION_HINT_TEXT_GAP, arrowTailY - this.font.lineHeight * CUSTOMIZATION_HINT_TEXT_SCALE * 0.5F + CUSTOMIZATION_HINT_TEXT_Y_OFFSET, 0.0D);
    ...
    private static void drawCustomizationHintArrow(GuiGraphics graphics, float tipX, float tipY, int color) {
        graphics.pose().pushPose();
        graphics.pose().translate(tipX, tipY, 0.0D);
        graphics.pose().mulPose(Vector3f.ZP.rotation(CUSTOMIZATION_HINT_ARROW_ROTATION));
