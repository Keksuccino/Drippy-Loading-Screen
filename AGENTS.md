...
- This project is "Drippy Loading Screen" (aka. "Drippy"), which is a Minecraft Java 26.2 mod that uses the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric` and `neoforge`.
- It also has a special `earlywindow` submodule for NeoForge's "Early Loading Screen".
- Drippy is an addon for "FancyMenu", which is a mod to customize Minecraft's menu screens. Drippy expands FancyMenu to support the loading overlay that shows up when launching the game and when reloading resources.
...
## Networking & Packets
- FancyMenu uses its own custom packet system, and Drippy should use that custom packet system when it needs to add packets.
