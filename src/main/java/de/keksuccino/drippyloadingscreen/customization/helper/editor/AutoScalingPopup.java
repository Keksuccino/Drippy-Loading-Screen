...
public class AutoScalingPopup extends FHPopup {
    ...
    public void render(PoseStack matrix, int mouseX, int mouseY, Screen renderIn) {
        ...
        drawCenteredString(matrix, font, Locals.localize("drippyloadingscreen.helper.editor.windowsize.currentwidth") + ": " + Minecraft.getInstance().getWindow().getScreenWidth(), screenCenterX, screenCenterY + 45, -1);
        drawCenteredString(matrix, font, Locals.localize("drippyloadingscreen.helper.editor.windowsize.currentheight") + ": " + Minecraft.getInstance().getWindow().getScreenHeight(), screenCenterX, screenCenterY + 55, -1);
        ...
        try {
            if (MathUtils.isInteger(this.widthTextField.getValue()) && MathUtils.isInteger(this.heightTextField.getValue())) {
                ...
                if ((w > 0) && (h > 0)) {
                    ...
                    LayoutEditorScreen.displayNotification(StringUtils.splitLines(Locals.localize("drippyloadingscreen.helper.editor.properties.autoscale.error"), "%n%"));
                }
            } else {
                this.answerCallback(false);
                LayoutEditorScreen.displayNotification(StringUtils.splitLines(Locals.localize("drippyloadingscreen.helper.editor.properties.autoscale.error"), "%n%"));
